...
static unsigned long size = 4;
static uint64_t iteration_count = 100000;
static unsigned int thread_count = 10;
...
run_test (void * arg)
{
...

  {
    ...
    for (i = 0; i < total_iterations; i++)
      {
	buf[i] = wfmalloc (request_size, tid);//malloc(request_size);//
      }

    for (i = 0; i < total_iterations; i++)
      {
    	wffree (buf[i]);//free(buf[i]);//
      }
  }
